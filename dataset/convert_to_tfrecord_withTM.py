...
ref_TM_path = "tone_mapped_aligned.tif"
...
for i, scene_dir in enumerate(scene_dirs):
    ...
    try:
        ...
        ref_TM = cv2.imread(os.path.join(cur_dir, ref_TM_path)).astype(np.uint8)
    ...
    def write_example(h1, h2, w1, w2):
        ...
        ref_TM_patch = ref_TM[h1:h2, w1:w2, ::-1]
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
			...
			'ref_TM':bytes_feature(ref_TM_patch.tostring()),
            }))
        writer.write(example.SerializeToString())

    # generate patches
    for h_ in range(250, h-patch_size+1, patch_stride):
        for w_ in range(760, w-patch_size+1, patch_stride):
